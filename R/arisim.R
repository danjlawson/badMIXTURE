##########################
## DATA
#' @title Simulated genetic similarity matrices
#'
#' @description
...
#'
#' This dataset was simulated to understand the Ari people, who are culturally split into "Blacksmiths" and "Cultivators". There was much discussion whether the Blacksmiths were a better representative of an ancient African population, or whether they were simply marginalised and hence had experienced genetic drift that made them look different to other populations. Package \code{\link{mixChecker}} allows you to test exactly this.
#' 
#' This simulation follows the so-called "Marginalisation" scenario, in which the truth is that the "Ari Blacksmiths" (Population 13) are drifted, whilst the "Ari Cultivators" (population 5) are the best representative of the ancestral population. The mixture solution (\code{arisim$mix} produced with ADMIXTURE, Alexander, Novembre & Lange 2009, Genome Research 19:1655-1664) gets this wrong but the residual plots (\code{arisim$data} produced with ChromoPainter, Lawson, Hellenthal, Myers & Falush 2012, PLoS Genetics, e1002453)  highlight this.
#'
#' See \code{\link{compareMixtureToData}}) and the examples within for how to understand this data.
#' 
#' @format arisim contains the following items:
#' \itemize{
...
###############################################
###############################################
#' @title Small dataset for experimentation
#'
#' @description
#'
